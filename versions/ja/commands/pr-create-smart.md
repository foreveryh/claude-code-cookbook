...
git コマンドの実行前に、現在ディレクトリが Git リポジトリか確認します。リポジトリ外では PR 手順をスキップし、初期化手順を案内してください（自動 `git init` は行わない）。

```bash
# ガード：Git リポジトリ外ならスキップ
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  echo "Git リポジトリ外のため、PR 作成ステップをスキップします。"
  echo "正しいプロジェクトで git init・リモート追加・コミットを済ませてから /pr-create-smart を実行してください。"
  exit 0
fi
```
