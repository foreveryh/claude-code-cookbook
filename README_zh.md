...
- **[Claudelog](https://claudelog.com)** - Claude Code 的全面指南、教程和最佳实践
...
|| `/pr-create-smart` | 基于改动生成高质量 PR 描述草稿，配合现有流程创建 PR。注意：PR 创建仍需通过 `gh` 或托管平台 UI 完成。 |
| `/pr-feedback` | 高效处理 Pull Request 审查意见，通过三阶段错误分析方法解决根本问题。 |
| `/pr-check` | PR 前质量与安全检查清单。 |
| `/test-e2e-local` | 本地拉起 E2E 验证关键路径。 |
